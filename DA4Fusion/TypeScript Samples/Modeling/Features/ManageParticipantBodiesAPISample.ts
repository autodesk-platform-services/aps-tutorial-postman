...
function run() {
  ...
  const sketchForCut = sketches.add(rootComp.xZConstructionPlane)
  const sketchForCutCircles = sketchForCut.sketchCurves.sketchCircles
  sketchForCutCircles.addByCenterRadius(centerPoint, 1.5)
  
  // Get the profile defined by the circle.
  const profForCut = sketchForCut.profiles.item(0)
  ...
  const distanceExtentDef1 = adsk.fusion.DistanceExtentDefinition.create(distance)
  extInput.setOneSideExtent(distanceExtentDef1, adsk.fusion.ExtentDirections.PositiveExtentDirection)
  ...
  const quantityOne = adsk.core.ValueInput.createByString('1')
  const distanceOne = adsk.core.ValueInput.createByString('0 cm')
  const quantityTwo = adsk.core.ValueInput.createByString('6')
  const distanceTwo = adsk.core.ValueInput.createByString('15 cm')
  ...
  const patBodies = rectangularFeature.bodies
  const body0 = patBodies.item(0)
  const face0 = body0.faces.item(0)
  const extCutInput = extrudes.createInput(profForCut, adsk.fusion.FeatureOperations.CutFeatureOperation)
   
  // Set the extrude input
  const distanceForCut = adsk.core.ValueInput.createByString('90 cm')
  const distanceExtentDef2 = adsk.fusion.DistanceExtentDefinition.create(distanceForCut)
  extCutInput.setOneSideExtent(distanceExtentDef2, adsk.fusion.ExtentDirections.PositiveExtentDirection)
